...
investigate something fishy going on with one particular air monitor in 
northwest Pasadena.

Let's pull it up by its monitor label, *BikeSGV - West Pasadena*, from a loaded 
...
While this surface level view may seem normal at first, the monitor's past
...
This monitor's A and B PM sensors appear to be telling us two very different 
...
Just for reference, this is the scatterplot for a different monitor not 
experiencing this strange issue:

```{r normal_scatterplot, warning=FALSE, message=FALSE}
pat_normal <- pat_loadLatest(pas, "NW of Claremont Village", 
                             startdate="2019-06-18", enddate="2019-06-25")
...
The Pasedena monitor is quite the opposite. Let's take a closer look at how its 
...
pat_april <- pat_loadLatest(pas, "BikeSGV - West Pasadena", 
