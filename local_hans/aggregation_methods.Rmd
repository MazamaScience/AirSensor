...
You can create different aggregation periods by explicitly providing `unit`, a string describing the period to split by `unit =` `'hours'`, `'minutes'`, `'weeks'`, `'months'`, etc. and `count`, a number of periods to aggregate in each bin. For example, a 15-minute standard deviation (`sd`) aggregation would look as follows:
