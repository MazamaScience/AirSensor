...
### airsensor
* The directory must have the following structure: ~airsensor/YYYY.

* File naming must be airsensor_scaqmd_YYYYMM.rda

* Example: ~airsensor/2020/airsensor_scaqmd_202009.rda

### pas
* The directory must have the following structure: ~pas/YYYY.
...
### pat
* The directory must have the following structure: ~pat/YYYY/MM.
...
outputDir <- paste0(archiveDir,"/airsensor/2020/")
...
for ( deviceDeploymentID in mvcaa_ids[1:idCount] ) {
  
  count <- count + 1
  print(sprintf("Working on %s (%d/%d) ...", deviceDeploymentID, count, idCount))
...
tryCatch(
  expr = {
    ...
    filename <- paste0("airsensor_", opt$collectionName, "_", "202009", ".rda")
...
#FATAL [2021-03-19 10:35:18] Error creating monthly AirSensor file:  Error: $ operator is invalid for atomic vectors

# error due to list containing $, don't know how to fix it 
# i tryied loading pat files from local dir and base url. 

# _________________ remove later _______________________________________________
# try to manually combine pat objects to see if pat_createAirSensor() works 
DATA_DIR <- "C:/Users/astri/Mirror/Mazamascience/Projects/AirSensor/Data/MVCAA" #remove later
...
mvcaa_ids # take 2 ids from this list 

pat1 <- pat_load(id = "96b108298883ca47_64441",
           pas = mvcaa, 
           startdate = 20200901,
           enddate = 20200930, 
           timezone = "America/Los_Angeles")
pat1 <- pat_createAirSensor(
  pat = pat1,
  parameter = "pm25",
  FUN = PurpleAirQC_hourly_AB_01,
)

pat2 <- pat_load(id = "f96deab8c29aa42b_10134",
           pas = mvcaa, 
           startdate = 20200901,
           enddate = 20200930, 
           timezone = "America/Los_Angeles")
pat2 <- pat_createAirSensor(
  pat = pat2,
  parameter = "pm25",
  FUN = PurpleAirQC_hourly_AB_01,
)
...
air_test <- PWFSLSmoke::monitor_combine(air_list) #OK, this creates an airsensor object 

air_test_meta <- air_test$meta # explore meta 

# save and then load the airsensor object and file to see if it works 
save(air_test, file = paste0(outputDir, "airsensor_", "scaqmd_" , "202009", ".rda")) # remove from directory 

airsensor_202009 <- sensor_loadMonth(
  collection = "scaqmd",
  datestamp = 202009,
  timezone = timezone
)
...
# try to load airsensor data from base url 
setArchiveBaseDir(NULL)
getArchiveBaseDir()
...
air_202009 <- sensor_loadMonth(
  collection = "scaqmd",
  datestamp = 202009,
  timezone = "America/Los_Angeles"
) %>% 
PWFSLSmoke::monitor_subset(monitorIDs = mvcaa_ids) 
# air_2009 has no data. I wonder if it's because the file does not contain mvcaa_ids sensors

# create a test list of sensors from the unfiltered airsensor object 
air_202009 <- sensor_loadMonth(
  collection = "scaqmd",
  datestamp = 202009,
  timezone = "America/Los_Angeles"
...
air_202009$meta$deviceDeploymentID

list_test <- c("4565cea0bf1881e5_5238",  "7469f3846aa28fbb_5236",  
               "c6f6ba9a91fd90a6_5220",  "f41bea1bac733fb6_5044")

# subset using list_test
air_test <- sensor_loadMonth(
  collection = "scaqmd",
  datestamp = 202009,
  timezone = "America/Los_Angeles"
) %>% 
PWFSLSmoke::monitor_subset(monitorIDs = list_test) 

air_test$meta$deviceDeploymentID #"4565cea0bf1881e5_5238" "7469f3846aa28fbb_5236" "c6f6ba9a91fd90a6_5220" "f41bea1bac733fb6_5044"
# it worked 


# check if Methow Valley IDS are in air_202009
mvcaa_ids
air_202009_meta <- air_202009$meta
#Methow Valley sensors are not listed in air_202009. Why?
#______________________________________________________________________________

# (for later) save airsensor data locally 
save(air_202009, file = paste0(outputDir, "airsensor_", "scaqmd_" , "202009", ".rda"))


# (for later) load airsensor file from local archive 
setArchiveBaseDir(archiveDir)
getArchiveBaseDir()
...
airsensor_202009_meta <- airsensor_202009$meta
