...
for ( monthStamp in monthStamps ) {
  ...
  year <- strftime(startdate, "%Y")
  month <- strftime(startdate, "%m")
  ...
  dir.create(
    file.path(archiveDir, "pat", year, month), 
    showWarnings = FALSE,
    recursive = TRUE
  )
  ...
  for ( id in mvcaa_ids ) {
    ...
    fileName <- paste0("pat_", id, "_", year, month, ".rda")

    # Create PAT canonical file path
    filePath <- file.path(archiveDir, "pat", year, month, fileName)
  ...
  dir.create(
    file.path(archiveDir, "airsensor", year), 
    showWarnings = FALSE,
    recursive = TRUE
  )
  ...
  tryCatch(
    expr = {
      ...
      filePath <- file.path(archiveDir, "airsensor", year, fileName)
