...
This tutorial demonstrates how to create a multi-MM archive of PurpleAir
...
for a single MM, this tutorial provides a more organized approach for working
...
and `sensor_load()` functions. We will build a multi-MM archive for the 
...
For a multi-YYYY archive, the overall structure beneath the `archiveBaseDir` 
will look something like this, depending on how many YYYYs of data you have:
...
The prtocol for creating pre-generated edata files depends on the type of object
being created.

### airsensor

* Directory: `<archiveBaseDir>/airsensor/<YYYY>/`
* File name: `airsensor_<collection>_<YYYYMM>.rda`
...
archive is searched for at the URL specified with `setArchiveBaseUrl()`. Mazama
...
DATA_DIR <- "C:/Users/astri/Mirror/Mazamascience/Projects/AirSensor/Data2/MVCAA"
...
for ( monthStamp in monthStamps ) {
  ...
  # Guarantee that the enddate is the first of the next MM
  ...
  collectionName <- "mvcaa" # user defined 
  ...
  # Loop over all ids and aggregate to hourly (might take a while).
  for ( id in mvcaa_ids ) {
  ...
  tryCatch(
    expr = {
      ...
      fileName <- paste0("airsensor_", collectionName, "_", monthStamp, ".rda")
      
      # Create Airsensor canonical file p ath
      filePath <- file.path(archiveDir, "airsensor", YYYY, fileName)

      save(list = "airsensor", file = filePath)
    }, 
    error = function(e) {
      msg <- paste("Error creating MMly AirSensor file: ", e)
      logger.error(msg)
    }
  )

  # Now proceed to the next MM
}
```

## Loading Local Data

Now we have a local archive of data created by following the directory and file
name protocol. We can now work with this archive by running
`setArchiveBaseDir()`.
...
# However, data for a specific sensor might not be available for a given MM.
# You should make sure to have the pat file in your MM directory as well.
