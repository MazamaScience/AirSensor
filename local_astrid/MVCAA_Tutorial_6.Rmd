...
The goal in this tutorial is to show how to evaluate the performance of PA sensors
under extreme environmental conditions associated with high PM25 concentratons
...
which together burned over 410,000 acres of grass and shrub throughout the month.
...
across the state (See: https://wasmoke.blogspot.com/2020/09/). September was
the smokiest month during the Fall season as shown in the timeseries 
plots below. 

## Data and Graphics 

Let's begin our journey by plotting and comparing the hourly values with daily 
averages for all sensors in the Methow Valley during September 2020.
...
# Load snesor data from our archive 
mvcaa_sensors_fall <- 
  sensor_load(
    collection = "mvcaa",
    startdate = 20200901,
...
We can use this spatial metadata for Twisp to find all sensors within 10km.
...
The QC algorithm [AB_01](https://mazamascience.github.io/AirSensor/reference/PurpleAirQC_hourly_AB_01.html) 
that we used into the function `pat_createAirSensor()` in
...
for each sensor by applying the QC algorithm 
[AB_00](https://mazamascience.github.io/AirSensor/reference/PurpleAirQC_hourly_AB_00.html)
and replotting the data. But first, let's explore the raw data by running 
`pat_internalFit()` for a quick assessment.
...
the PM25 concentrations. Only the Benson Creek sensor had multiple days with missing data.

Now we can compare each sensor performance against the Twisp monitor by fitting
...
have an r-squared value > 0.9 to be considered reasonable.

We will make the executive decision that all of these sensors are performing
acceptably and will use the more lenient AB_00 QC algorithm to create 
...
A shorter distance _should_ result in a better fit as long as both sensor and
monitor are at a similar elevation. 
...
Looking at the map we notice that Balky Hill is the closest sensor to 
monitor in Twisp. We can also see that there are some significant hills
...
we can replot the daily barplots. 
...
PA sensors can be valuable tools to evaluate PM2.5 concentrations in the air but 
require a careful assessment to understand if they work properly. As we saw, the 
**AirSensor** package offers functions to monitor the ongoing and past state of 
...
and how to interpret and visualize the data in effective ways. 

At the end of the day, it's up to the individual to decide which QC algorithm to 
apply and which sensors to include in your study based on the overall sensor 
performance, a review of days with missing data, internal and external fits, and 
consideration of the eographic features surrounding the sensors.
