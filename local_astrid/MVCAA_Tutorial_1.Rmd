...
??geterrormessage() #try geterrmessage()
...
list.files("~/Data/mvcaa") #character(0), this doesn't work. Can't access directory. Can't use path, only folder.
list.files("~/Data") # this works: [1] "MVCAA"  

# Load files
mvcaa <- get(load("~/Data/mvcaa/mvcaa.rda")) #this doesn't work, need to reset archiveDir
archiveDir <- "Data/MVCAA"
archiveDir <- paste0(wd, "/", archiveDir)

# Examine archive directory again
list.files(archiveDir) #"mvcaa.rda"   "patList.rda". OK!

# Load files
mvcaa <- get(load(paste0(archiveDir,"/mvcaa.rda")))
patList <- get(load("~/Data/mvcaa/patList.rda")) #this doesn't work
patList <- get(load(paste0(archiveDir,"/patList.rda"))) #OK!
