...
Run `path.expand("~")` in the R console to see the locaiton of your home directory. 
...
# part ("~") and keeping the "/Data/MVCAA".
#   Windows example: "C:/MethowValley/Data/MVCAA" 
#   UNIX example: "~/Data/MVCAA" works well on UNIX/OSX.!to be removed!
DATA_DIR <- "~/Data/MVCAA" 
```
Jon, I think the code above should be removed: "UNIX example: "~/Data/MVCAA" works well on UNIX/OSX".
"~" works well with Windows as well, if one wants to create a data directory under the 
home directory "~". 
...
zooming in on the Methow Valley.
...
Ambassador". With this information, we can now create a script to create 'pat' 
...
Now we have a local copy of all of the 'pas' and 'pat' data we wanted, pre-compiled
