...
observations. We can explore further if the difference between `pm25_A` and 
`pm25_B` data are significantly different based on a student’s t-test. 

```{r, PurpleAirSoH_dailyABtTest() - Willowbrook_Farm}
tbl <- 
  Willowbrook_Farm %>%
...
### I " hover near 0" should be substituted with an alpha value since we are 
### analyzing p-values. How can we reject or fail to reject the Null hypothesis 
### without alpha? (updated issue #261)

```{r, PurpleAirSoH_dailyABtTest() - Balky_Hill}
tbl_Balky <- 
  Balky_Hill %>%
  PurpleAirSoH_dailyABtTest()
