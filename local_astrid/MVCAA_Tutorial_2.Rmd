...
_NOTE:_ You can expriment with the `rollPeriod` by adjusting the number in the
dygraph viewer.
...
As before, subsampling to only 1000 points speeds up plotting.

_NOTE:_ This plot carefuly retains outliers in the A and B channels while 
...
ones. The plot aove shows a few non-concerning outliers. Outliers of concern may be
...
(slope = 1.5), as is aslo evident in the bottom graphic, indicating the 
...
function is in anotgher new category of functions that compare hourly aggregated sensor 
data with data from a professionally maintained sensor.
...
or is not within the `distanceCutoff`, it is possible that you will ecounter the following error: 
...
recommended distance to ensure that both instruments are measuing similar parcels
of air.

To learn more info about the PWFSL monitors in your area of interest and 
...
* in grey -- sensor data before aggregation and QC (PA raw)
* in purple -- sensor data after the QC treatment (PA hourly)
* in balck -- data collected by the nearest onitor (Monitor)
...
```{r patexternalFit-01-Liberty-Schooll, eval = TRUE, warning = FALSE, message = FALSE}
...
```{r patexternalFit-00-Liberty-Schooll, eval = TRUE, warning = FALSE, message = FALSE}
# create comparison plot for Liberty School sensor
