...
detectors — one labeled A and one labeled. B. All of the plotting functions 
except `pat_monitorComparison()` and `pat_externalFit()` only use data from a 
single PA sensor — with A and B data channels. When measurements from the A and 
B channels are largely in agreement then the sensor is working properly. When 
they differ, there is a problem. Moreover, when a sensor is properly 
functioning, the only correlations will be a strong positive one between 
the A and B channels (pm25_A:pm25_B) and a strong negative one between 
