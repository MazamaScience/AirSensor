...
Both graphs show missing data for one of the two channels on certain days, but this is not a major concearn given the isolated episodes.
For Liberty School, measurements from the A and B channel are largely in agreement 
...
# ! 
### Jon, I can't link the function because the doc online doesn't work (error 404). I have already open an issue. 
...
This function detects outliers using a Median Average Deviation "Hampel" filter, appling a rolling Hampel filter to find those points that are very far out in the tails of the distribution of values within the window. 

`windowSize = 15` corresponds to a temporal window of approximately 30 minutes.

The default threshold setting `thresholdMin = 8` identifies points that are extremely unlikely to be part of a normal distribution and therefore very likely to be an outlier.
...
In order to store the new dataset including the replaced values, you can create a new sensor object and save it in your archive directory (already defined in 
Tutorial 1) like in the example below. 
...
lm_Liberty_School <- pat_internalFit(
  pat = Liberty_School,
  showPlot = TRUE,
  size = 1,
  a_color = "red",
...
# not included in final tutorial. 
tbl <- 
  Willowbrook_Farm %>%
...
### is this actually true? What's the alpha value generally used? I did the same
...
timeseriesTbl_multiPlot(tbl_Liberty)
hist(tbl_Liberty$pm25_A_pm25_B_p_value)
sum(tbl_Liberty$pm25_A_pm25_B_p_value < 0.05) #25 days of significant differences 
```
...
functioning sensor under normal conditions. Wildfire events during summer and the 
heavy use of heating systems during winter could explain oscillating R squared values. 
The `pm25_A_pctDC` plot shows that channel A
recorded dynamic data (value > 0) towards the beginning and the end of the month
in contrast to channel B, which collected sticky values throughout the entire month. This also corresponds to the period when the R squared values were lower as shown in
in the `pm25_A_pm25_B_rsquared` plot. Especially at the beginning of the month, the
plot shows R squared values hovering around zero, indicating the poor functioning of
the sensor.
