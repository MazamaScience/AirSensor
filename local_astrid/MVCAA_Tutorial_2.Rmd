...
# !
### this need some clarification. replace = TRUE does not replace all outliers. Also, why I can't calculate the mean of the observations (e.g. `mean(anysensor$data$pm25_A)` returns NA). I checked and the column is numeric. 
...
linear model using the `base` function `summary()`. And because this is a linear 
model, you can run your favorite statistical tests to investigate all possible 
distribution parameters. Set `showPlot = TRUE` if you
...
(slope = 1.5), which is also supported by the bottom graphic, indicating the 
malfunctioning of one or both of the Willowbrook Farm sensor's channels 
throughout September 2020. 
Additionally, we observe that the model explains ~ 92% of the data variability 
(R2 = 0.922).
