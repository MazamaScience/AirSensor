...
knitr::opts_knit$set(root.dir = "C:/Users/astri/Mirror/Mazamascience/Projects/AirSensor") #remove
```
...
```{r, eval=FALSE}
# Load files
archiveDir <- "C:/Users/astri/Mirror/Mazamascience/Projects/AirSensor/Data/MVCAA" #remove
...
The model explains only 92.2% of the data variability, supporting the assumption
that the sensor didn't function very well during September 2020. 

```{r, PurpleAirSoH_dailyABtTest() - Willowbrook_Farm, eval= FALSE}
# not included in final tutorial. remove
tbl <- 
  Willowbrook_Farm %>%
...
for `pm25_A_pctDC`, `pm25_B_pctDC`. A high percent DC (direct current) value 
...
# temporary notes to be removed in final tutorial. #remove
...
the persistent spikes throughout the month. Values are expected to hover near 0 for 
...
In this case, it's possible that the spikes recorded are due to wildfire events 
around that period.
...
#!
### it would be helpful to have a few sentences describing the graphs, and what are the minumum expected values for slope and R sq. Here we are comparing PA sensor data to Monitor data, how tolerant can we be in assessing their relationship?

### I think the function uses the default qc_algorithm value despite I specify different ones. lm_01 and lm_00 look alike, which is not possible. 

```{r, patexternalFit - Liberty_Schooll, eval= FALSE}
# create comparison plot for Liberty School sensor
