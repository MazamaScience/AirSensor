...
The `pat_aggregate()` function is used in the conversion of the *pat* time axis into data with a uniform, typically hourly, time axis. The aggregation process involves creating regular time bins and then calculating a variety of statistics for the data within each bin utilizing a user provided function.

Applying our uncontroversial `pat_qc()` as the first step in the pipeline, we
can review which statistics are calculated:
...
is available through a series of
...
```{r AB_boxplot_2, echo = FALSE, warning=FALSE}
library(ggplot2)
...
# Compare the t-statistic of channels A and B for that day
agg_data <-
  example_pat_failure_B %>%
  pat_filterDate(20190612,20190613) %>%
  pat_aggregate(FUN = function(x) { 
    tt <- t.test(x$pm25_A, x$pm25_B)
    return(c('p.value' = tt$p.value))
  }, level  = 1)

ggplot(agg_data, aes(datetime, p.value)) + 
  geom_point() +
  scale_y_log10() + 
...
hours where the A and B channels have hourly averages that are both highly
...
is the hourly averaged dataset that can be used for comparison with co-located
