...
"internet of things" platform where sensor readings can be stored and retrieved.
...
PM concentration readings are given by two
...
each providing its own separate reading channel (A and B). These optical 
particle counters function by measuring the amount of light scattered by
suspended particulate matter. A PMS5003 unit sends a laser through air sucked in
by a fan, detects the light reflected, and counts the number of different-sized 
particles based on 
...
In this process, however, calculating concentration relies on assuming an 
average particle density since not all particles have the same 
[composition](https://en.wikipedia.org/wiki/Particulates#Composition). 
This assumption is noted to be a potential cause for PurpleAir monitors reading 
higher than federal monitors.
...
seconds instead of over the past 24 hours, which may appear exaggerate the
real-time data since PM levels can fluctuate a good deal thoughout the day. 
[Source](https://www2.purpleair.com/community/faq#!hc-how-do-purpleair-sensors-compare-to-regulatory-particulate-matter-sensors)
