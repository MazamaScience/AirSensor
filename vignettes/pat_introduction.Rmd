...
Notice that when passing our synoptic dataframe "`pas`" to `pat_loadLatest()`, 
...
`pat_loadLatest()` with a start and end date will return  the most recent week 
of time series data.
...
The high R^2^ value says that the two channels are highly correlated while a slope
of ~0.9 says that slight relative bias in the measurements. (Perfect alignment
