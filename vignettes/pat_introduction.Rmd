...
monitor of interests label. -- UNCLEAR
...
Notice when passing our synoptic dataframe "`pas`" to `pat_load`, we also supply
...
PM2.5, it is unwieldy to analyze if we are only interested in anomolous air 
quality. We can use `pat_subdate` to subset our `pat` dates. In this case, we'll
reduce our DOI to August 2018.

```{r}
pat_august <- pat_subdate(pat = pat, sd = 20180801, ed = 20180901)
pat_multiplot(pat = pat_august, plottype = "pm25")
```

Our reduced dataframe `pat_august` may be a smaller time interval but 
intermintent sensor errors still likely exist. In order to identify and remove 
PM2.5 outliers we can use `pat_findOutliers`. We'd also like to display the
outliers to compare. Each outlier will be marked with a red *. 

```{r}
pat_findOutliers(pat = pat_august, showPlot = TRUE)
```
