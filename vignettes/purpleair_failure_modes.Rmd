...
The A channel is centered around a particular level but is sometimes effected
by humidity when it goes past a certain threshold.

```{r, echo=FALSE, warning=FALSE}
pat <- pat_loadLatest(pas, "SCEM_05", startdate = 20190701, enddate = 20190710)
pat_multiplot(pat)
```

The multiplot shows the A and B channels have very different readings, similar 
the previous failure mode. The strange thing here though is that the A channel
is mostly flat, with only the occasional spike when the humidity measures very 
high. Let's see what number it is centered around:
