---
title: "Outlier Detection with Hampel Filters"
...
using all neighboring values within a window of size `n`. If the point of 
interest lies multiple standard deviations from the median it is flagged as an 
outlier. As the 
...
* `n` -- number of measurements to include in a window
* `thresholdMin` -- number of std. dev. units above which a measurement is
considred an outlier

The default setting of the window size `n = 23` means that 23 samples
...
We definitely have what look to the eye like outliers so let's use the
...
# Default outlier replacement
pat2Day %>%
  pat_outliers(n = 23, thresholdMin = 8, replace = TRUE) %>%
...
# Aggressive outlier replacement 
pat2Day %>%
  pat_outliers(n = 23, thresholdMin = 4, replace = TRUE) %>%
  pat_multiplot("pm25", sampleSize = 1e6)
```
