...
4) invalidate pm25 hourly values where the absolute value of the t-test 
statistic is above some threshold (e.g. 100)
...
The R code to apply this QC would be:

```{r example_qc, eval = FALSE}
hourlyData <-
  example_pat_failure %>%
...
As a test of the overall effect, here are some hourly barplots for each of 
the three examples we started with:
...
from `example_pat_failure_B`. The four questionably high values in
`example_pat_failure_B` are all adjacent to an hour that has been flagged
as invalid which should call the validity of those values into question as well.
