...
data". The second example shows data that were clearly collected by a 
malfunctioning sensor. We will call that "bad data". Notice the jump in channel 
...
determine which of these datasets are more reliable without actually looking at 
...
all the time. Notice however, that for both stations the first few points hang
...
to June, all stations were recording at a higher rate, hence percentages greater
than %100. Before firmware updates were installed, a perfectly performing sensor
should report at %150.

Both stations start out with near full reporting percentages, and after the 
firmware update the good data are still nearly perfect, hovering at 100%
without any dropouts, but the bad data start to deteriorate between June 10 and 
June 17.
...
```{r, SoH_plot,  fig.width = 7, fig.height = 8.5}
...
from a sensor should be trusted. Questions that a data anylist would ask 
