...
The column numbers aren't identical, but the 'v's have been renamed to 'pm25...'. In addition, the final five columns are new. 

# Spatial Metadata 

These five columns all encode spatial metadata, information about the location of each sensor beyond the raw latitude and longitude coordinates that is in human-informative format. `enhanceSynopticData` uses the **MazamaSpatialUtils** package and each sensor's latitude and longitude to add columns with relevant and interesting information including the ISO 3166-1 alpha-2 country codes, and the ISO 3166-2 alpha-2 state codes and timezone. Even more interesting, it also supplies the distance to the nearest PWFLS monitor and that monitor's ID.  
