...
#
# docker run --rm -v /Users/jonathan/Projects/MazamaScience/AirSensor/local_executables:/app -w /app mazamascience/airsensor /app/createPAS_archival_exec.R 
#
...
if ( opt$version ) {
  cat(paste0("createPAS_archival_exec.R ", VERSION, "\n"))
  quit()
}
...
logger.setup(
  traceLog = file.path(opt$logDir, "createPAS_archival_TRACE.log"),
  debugLog = file.path(opt$logDir, "createPAS_archival_DEBUG.log"), 
  infoLog  = file.path(opt$logDir, "createPAS_archival_INFO.log"), 
  errorLog = file.path(opt$logDir, "createPAS_archival_ERROR.log")
)
...
# Start logging
logger.info("Running createPAS_archival_exec.R version %s",VERSION)
...
result <- try({
  ...
  filename <- paste0("pas_", timestamp, "_archival.rda")
  filepath <- file.path(opt$outputDir, filename)
...
if ( "try-error" %in% class(result) ) {
  msg <- paste("Error creating daily PAS archival file: ", geterrmessage())
  logger.fatal(msg)
} else {
