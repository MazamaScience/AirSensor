...
if ( opt$version ) {
  ...
  cat(paste0("createLatestWind_exec.R ",VERSION,"\n"))
  quit()
...
logger.setup(
  traceLog = file.path(opt$logDir, "createLatestWind_TRACE.log"),
  debugLog = file.path(opt$logDir, "createLatestWind_DEBUG.log"), 
  infoLog  = file.path(opt$logDir, "createLatestWind_INFO.log"), 
  errorLog = file.path(opt$logDir, "createLatestWind_ERROR.log")
)
...
# Start logging
logger.info("Running createLatestWind_exec.R version %s",VERSION)
...
result <- try({
  ...
  WD_filename <- paste0("airsensor_WD_latest07.rda")
  WD_filepath <- file.path(opt$outputDir, WD_filename)
  
  WS_filename <- paste0("airsensor_WS_latest07.rda")
